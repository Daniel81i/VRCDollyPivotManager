...
| 方式 | 動作 |
|---|---|
| **`oscquery`** | mDNS で自分を広告し、VRChat に見つけてもらう。**`receive_port` は使いません**（空きポートを取り、その番号を mDNS で伝えます）。加えて VRChat へ現在値を問い合わせられる |
...
#### 待ち受けポートを方式ごとに分けている理由

UDP リピーターで VRChat の OSC を複数のツールへ配っている場合、**両方式で同じ番号を使うと同じ配信を二重に受け取ります。** リピーターが配る分と、OSCQuery を見た VRChat が直接送ってくる分です。

Confirm は立ち上がりで検出しているので同時に2つ届く分には耐えますが、**2系統がずれて届くと立ち上がりが2回成立し、1回の押下で2本生成されます。**

`oscquery` のときに空きポートを取るのはこれを避けるためです。リピーターが配る番号と重なりません。`receive_port` はフォールバック専用として空けておけます。
